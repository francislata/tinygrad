...
def sliding_window_inference(model, inputs, labels, roi_shape=(128, 128, 128), overlap=0.5, batch_size=1):
  ...
  for i in range(0, strides[0] * size[0], strides[0]):
    for j in range(0, strides[1] * size[1], strides[1]):
      for k in range(0, strides[2] * size[2], strides[2]):
        input_buffer[count] = inputs[..., i:roi_shape[0]+i,j:roi_shape[1]+j, k:roi_shape[2]+k]
        count += 1
  ...
  for i in range(0, strides[0] * size[0], strides[0]):
    for j in range(0, strides[1] * size[1], strides[1]):
      for k in range(0, strides[2] * size[2], strides[2]):
        result[..., i:roi_shape[0]+i, j:roi_shape[1]+j, k:roi_shape[2]+k] += output_buffer[count] * norm_patch
